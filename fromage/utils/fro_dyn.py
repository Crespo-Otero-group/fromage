...
def dynamics_sequence(traj):
    ...
    shell_atoms: list<Atom>
        List of Atom objects for each atom in the low layer
    state : int
...
####################################################
    ...
    def __init__(self,init_dict, mol_atoms, shell_atoms):
        ...
        self.R = init_dict["R"]
        self.M = np.reshape(init_dict["M"],(-1,1)) * 1822.8852
        self.V = init_dict["V"]
...
#            self.Ekin = np.sum(0.5 * np.rot90(self.M) * np.sum(np.square(self.V),axis=1))
            self.Ekin = np.sum(0.5 * (self.M * self.V ** 2))
