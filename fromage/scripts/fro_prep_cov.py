...
import fromage.utils.z_scheme as zs
from fromage.utils.mol import Mol

def calculate_rms_and_max_gradient(gradients):
    """
    Check RMS for comparison with Gaussian thresholds
    """
       

    # Calculate RMS
    rms = np.sqrt(np.mean(np.square(gradients)))

    # Calculate max gradient
    max_gradient = np.max(np.abs(gradients))

    return rms, max_gradient
...
#    output_file.write("Read " + str(len(cell)) + " atoms in cell_file\n")
    ...
    region_1 = rf.mol_from_file("mol.init.xyz")
    ...
    high_points = Mol([])
    #assign charges
    real_atoms = region_1 + region_2
    all_charges = rf.read_mwfn_charges("molden.chg")
    for charge, atom in zip(all_charges, real_atoms):
        if atom in region_2:
            atom.q = charge
            high_points.append(atom)
        else:
            atom.q
    ...
    low_points = region_2.copy()

    if not os.path.exists(molden_low):
        low_points = high_points.copy()
    else:
        low_points = Mol([])
        all_charges = rf.read_mwfn_charges("molden_low.chg")
        for charge, atom in zip(all_charges, real_atoms):
            if atom in region_2:
                atom.q = charge
                low_points.append(atom)
            else:
                atom.q

    if inputs['bool_zscheme']:
        output_file = open(here+"/prep.out", "a")
        output_file.write(f"Running Z-scheme: {inputs['z_scheme']}\n")
        output_file.close()
        real_region = region_1 + region_2
        real_region.set_bonding(inputs["bonding"], inputs["bond_thresh"])

        #output_file.write(f"Redistributing high-level charges with Z-scheme: {inputs['z_scheme']}\n")
        high_points = zs.redistribute_charges(region_1, high_points,real_region, inputs['z_scheme'], inputs['z_thresh'], sys_type="cov")
        #output_file.write(f"Redistributing low-level charges with Z-scheme: {inputs['z_scheme']}\n")
        
        low_points = zs.redistribute_charges(region_1, low_points, real_region,inputs['z_scheme'], inputs['z_thresh'], sys_type="cov")

    print("high_points", high_points.get_total_charge())
    ######
...
#    os.chdir(ml_path)
    if low_level_write == ef.write_tinker_temp:
        ...
        os.chdir(ml_path)
        low_level_write("ml.temp", [], low_points, os.path.join(here, "ml.template")) ##### MICHAEL EDIT 13/10/2023
        os.chdir(rl_path)
        low_level_write("rl.temp", region_2, [], os.path.join(here, "rl.template"))
